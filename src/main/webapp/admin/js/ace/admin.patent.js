jQuery.adminPatent = {
		...
		initSearchDataTable : function() {
			if (this.userDataTable == null) {
				this.userDataTable = $('#dt_table_view').dataTable({
					...
					"bProcessing" : true,
					"bSort" : false,
					"sAjaxSource" : $.ace.getContextPath() + "/admin/patent/list",
					...
					"aoColumnDefs" : [
						{
							...
							'fnRender' : function(oObj, sVal) {
								return " <button class=\"btn2 btn-info\" onclick=\"$.adminPatent.deletepatent("+oObj.aData.id+")\"><i class=\"icon-trash\"></i> 删除</button>";
							}
						},
					 {
						'aTargets' : [ '_all' ],
		...
		deletepatent :function(id){
			bootbox.confirm( "是否确认删除？", function (result) {
	            if(result){
	            	$.ajax({
	        			type : "get",
	        			url : $.ace.getContextPath() + "/admin/patent/delete/"+id,
	        			dataType : "json",
	        			success : function(json) {
	        				if(json.state=='success'){
	        					noty({"text":""+ json.msg +"","layout":"top","type":"success","timeout":"2000"});
	        					$.adminPatent.initSearchDataTable();
	        				}else{
		...
		showEdit: function (id){
			...
			$.ajax({
    			type : "get",
    			url : $.ace.getContextPath() + "/admin/patent/get/"+id,
    			dataType : "json",
    			success : function(json) {
    				if(json.state=='success'){
		...
		saveUser: function(id){
			$.ajax({
    			type : "post",
    			url : $.ace.getContextPath() + "/admin/patent/save",
    			data:$("form").serialize(),
    			dataType : "json",
    			success : function(json) {
    				if(json.state=='success'){
